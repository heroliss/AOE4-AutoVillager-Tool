...
《帝国时代4》自动化工具。**3.0 起，本项目主体是一套「节点式可视化自动化引擎」**：在网页编辑器里拖拽节点、连线，像搭积木一样组出自己的自动化流程（内置「统一生产：村民/乡骑/商人」开箱即用）。3.0 之前那套「固定写死的自动出农工具」(v2.x) 仍保留在仓库，见[文末附录](#附录旧版固定工具-v2x)。

> ⚠️ 需要管理员权限（用于操作期间的输入屏蔽）。模板/坐标默认基于 2560×1440 + 中国阵营 + HDR 关闭，其它分辨率/阵营在节点上重新框选区域、重截模板即可。

## 引擎是什么

把「截屏识别 → 判断 → 发按键/点击」这条自动化链路拆成可视化**节点**，用两种连线编排：

- **执行流**（白线，类 Unreal Blueprint）：决定"这一帧依次做哪些动作"。
- **数据流**（彩线）：节点间传值（如把"识别到的人口"喂给"产能计算"）。

节点分五类：**感知**（截屏/模板匹配/OCR）、**逻辑**（与或非/比较/算式/分支）、**操作**（按键/点击/输入屏蔽/编组锁）、**游戏**（三态遮挡检测/选中建筑计数/产能计算）、**控制**（每帧触发/条件/开关/延时）。流程存成 `*.flow.json`，纯数据节点按帧记忆化（整屏只截一次、多处切片复用）。

编辑器基于 **LiteGraph.js + pywebview**，支持：节点内直接编辑参数、框选区域/截取模板等采集工具、🖼运行预览（看节点"看到的画面"+置信度）、⏱耗时监控、断点单步、**分组折叠（组≈函数，可嵌套）**、控制面板置顶常用旋钮、游戏内半透明覆盖层（启停/调参/资源监控）。
...
python run_editor.py                      # 打开网页编辑器（新建空白流程）
python run_editor.py flows/combined.flow.json   # 打开内置「统一生产」流程

python run_headless.py                    # 无界面直接跑 flows/combined.flow.json（默认只开村民）
...
- **设置 → 用户界面 → 全局建造队列 → "全部显示"**：左侧常显生产队列图标，用于检测是否有单位在产。
- **设置 → 控制 → 选择所有城镇中心 → 绑定到 H 键**（多 TC 同时生产的前提）。出商人还需把「选择所有市场」绑到 J 键。

## 内置流程：统一生产（村民 / 乡骑 / 商人）

同一帧内依次生产三种单位，每段一个独立**开关**（默认只开村民）。前段做一次共享判断（修饰键暂停/窗口/遮挡）→ 生产门控（队列空? 资源够? 人口有空位? 短路判断，省去无谓的抢锁/屏蔽）→ 取锁/屏蔽/存编组 → 三段接力生产 → 统一收尾。人口与资源在多段间结成"预算链"逐段扣减，避免同帧重复占用。图按「组≈函数」两级嵌套，可折叠查看。

## 深入 / 二次开发

引擎的执行模型、核心抽象、全部节点目录、流程 JSON 格式、如何新增节点/流程，见 **[docs/架构与开发指南.md](docs/架构与开发指南.md)**。

## 目录结构（引擎）

```
flow/                  引擎本体
  core/                执行模型（图/执行器/上下文/节点基类）
  nodes/               节点库（sense/logic/action/game/control + _imaging 工具）
  flows/               内置流程的 Python 构建器（.py 为权威源，selfcheck 据此生成 .flow.json）
  editor/              网页编辑器（webhost.py + web/：LiteGraph 前端 + 采集/覆盖层）
  layout.py            自动排版（主线横向铺开 + 分组感知）
flows/combined.flow.json   内置「统一生产」流程（由 combined.py 生成）
templates/             模板图片（如 blocked.png 遮挡模板、单位/数字模板）
run_editor.py          启动编辑器          run_headless.py  无界面运行
selfcheck.py           自检 + 重新生成内置流程
docs/架构与开发指南.md     架构与二次开发文档
```
...
> 以下为 3.0 之前那套「固定写死的自动出农工具」的文档，仍保留可用（`python main.py` 命令行、`python gui_app.py` 图形界面，或打包的 exe）。它与上面的引擎相互独立。
