...
- 帧缓存（性能基础，类似"懒加载"）：capture_region 按区域缓存——某区域本帧第一次被需要时才截，
  之后同一区域直接命中缓存复用，无需任何"预取"节点。可选的 prefetch_full 整屏只抓一次，
  之后 capture_region 切片复用；但整屏截图在 Windows 下很贵（DWM 同步 + 整块位图拷贝），
  本工具的识别区域都很小，逐区域按需截通常更快，故默认不用它。
