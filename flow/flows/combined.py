...
def build_combined_graph() -> Graph:
    ...
    g.groups = [
        ...
        # 默认折叠成一个紧凑箱体：6 个传感器都是“带截图预览”的高节点，竖排会拉成一条贯穿整图的竖条
        # （压住别的组框）。折叠后只占一小块、输出端口汇到箱体上；需要核对截图区域时展开即可（同门控）。
        {"id": "g_sense", "title": "共享识别", "color": "#3f7a6a", "collapsed": True,
         "desc": "每帧只识别一次、多段共享：人口/空位/食物/黄金/木头/TC。展开可逐个核对截图区域。",
