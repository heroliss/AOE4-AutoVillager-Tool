...
def main() -> None:
    ...
    # 注：三段共享人口池(空位)，村民+乡骑共用食物池，乡骑+商人共用黄金池；商人吃木头60+黄金60(不吃食物)。
    #   各段按计划数排：村民3、乡骑2、商人2（c_per_cart 默认=2）。
    #   空位链 150→村民用3剩147→乡骑用2剩145→商人；
    #   食物链 300→村民用150剩150→乡骑(65×2)剩20（商人不吃食物）；
    #   黄金链 800→乡骑(50×2)剩700→商人(60×2)（700//60=11≥计划2）；木头 800→商人(60×2)（800//60=13≥计划2）。
    ...
    ok &= check("出商人 q x2 = min(2*1, 剩余空位145, 木头800//60=13, 剩余黄金700//60=11)", any("按键 q x2" in m for m in logs))
    ...
    print("== 统一生产：商人受木头限制（商人吃60木60金；木头只够1个）==")
    logs, _ = run_with_stubs(build_combined_graph,
                             {**COMBINED, "sw_vill": {"value": False}, "sw_xq": {"value": False},
                              "sw_cart": {"value": True},
                              "wood": {"value": 80, "value2": None, "ok": True}})  # 80//60=1
    ok &= check("商人木头只够1个 q x1 = min(计划2, 木头80//60=1, 黄金800//60=13)", any("按键 q x1" in m for m in logs))
    ok &= check("木头只够1个则不出2个商人", not any("按键 q x2" in m for m in logs))
